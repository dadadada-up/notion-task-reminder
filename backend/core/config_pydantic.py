"""
统一配置管理模块
优先使用 Pydantic 进行配置验证和类型检查
如果 Pydantic 不可用，则使用简化版本
支持从环境变量和 .env 文件加载配置
"""
...
# 尝试导入 Pydantic，如果失败则使用简化版本
try:
    from pydantic import BaseModel, Field, validator
    from dotenv import load_dotenv
    PYDANTIC_AVAILABLE = True
except ImportError:
    print("⚠️  Pydantic 不可用，使用简化版配置管理")
    PYDANTIC_AVAILABLE = False
    # 导入简化版本
    from .config_simple import (
        Settings, NotionConfig, PushConfig, EmailConfig,
        GitHubConfig, WebConfig, RuntimeConfig,
        get_settings, reload_settings
    )
    # 直接导出简化版本的函数
    __all__ = ['Settings', 'get_settings', 'reload_settings']

# 如果 Pydantic 可用，继续使用原来的实现
...
class PushConfig(BaseModel):
    """推送配置"""
    pushplus_token: Optional[str] = Field(None, description="PushPlus Token")
    wxpusher_token: Optional[str] = Field(None, description="WxPusher Token")
    wxpusher_uid: Optional[str] = Field(None, description="WxPusher UID")
...
class EmailConfig(BaseModel):
    """邮件配置"""
    enabled: bool = Field(False, description="是否启用邮件")
    smtp_server: Optional[str] = Field(None, description="SMTP 服务器")
    smtp_port: int = Field(465, description="SMTP 端口")
    sender: Optional[str] = Field(None, description="发件人邮箱")
    password: Optional[str] = Field(None, description="邮箱密码/授权码")
    receiver: Optional[str] = Field(None, description="收件人邮箱")
...
class WebConfig(BaseModel):
    """Web 服务配置"""
    port: int = Field(5000, description="服务端口")
...
class RuntimeConfig(BaseModel):
    """运行时配置"""
    reminder_type: str = Field("daily_todo", description="提醒类型")
    send_time: str = Field("08:00", description="发送时间")
    action_type: str = Field("combined", description="操作类型")
    debug_mode: bool = Field(False, description="调试模式")
    force_send: bool = Field(False, description="强制发送")
    
    @validator('reminder_type')
    def validate_reminder_type(cls, v):
        allowed = ['daily_todo', 'daily_done', 'both']
        if v not in allowed:
            raise ValueError(f'reminder_type 必须是 {allowed} 之一')
        return v
